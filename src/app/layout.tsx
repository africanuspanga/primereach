...
import { getSite, getContact, getNav } from "@/lib/content";
import { SITE, CONTACT, absoluteUrl } from "@/lib/constants";
...
export default async function RootLayout({
  ...
  const [site, contact, nav] = await Promise.all([
    getSite(),
    getContact(),
    getNav(),
  ]);
  ...
  return (
    ...
    >
      <body className="flex min-h-full flex-col bg-white text-ink">
        ...
        />
        <SiteHeader site={site ?? SITE} contact={contact ?? CONTACT} nav={nav.length > 0 ? nav : undefined} />
